...
## Overview
Implement a new feature from zero to production-ready, layer by layer. Use this
when adding a meaningful capability (a new API resource, a UI screen, a worker)
that touches multiple parts of the codebase and needs tests and docs.
...
- [ ] **2.1** Implement the business rules
  ...
  - Verify: add tests covering the happy path and at least one error case
...
## Phase 3: Surface
- [ ] **3.1** Expose the feature on the API / UI
  - Wire the service to handlers in `src/api/` (or components in `src/ui/`)
  - Verify: an end-to-end test hits the surface and returns the expected result
- [ ] **3.2** Add input validation at the boundary
  - Validate and sanitize every input; return clean errors for invalid requests
  - Verify: probe with malformed input and confirm a 400 with a helpful message

## Phase 4: Integration & polish
- [ ] **4.1** Integrate with existing features
  - Update anything that references the touched models/services to stay consistent
  - Verify: the full suite (`<your-test-command>`) passes with no regressions
...
## Acceptance criteria
1. `<feature>` works end-to-end from the surface down to the data layer.
2. Happy path, error cases, and edge cases all have passing tests.
3. The full suite is green with no regressions in existing features.
4. The feature is documented with its usage and test command.
